#!/usr/bin/env python
# coding: utf-8
"""Spherical pyCloudy model with internal line-of-sight attenuation.
...
from internal_attenuation import attenuated_profile, attenuated_projection
...
MODEL_NAME_PREFIX = "M3D_spherical_profiles_rhd_wind_temperature_attenuation"
DIM = 151
PROJ_AXIS = 0
...
LINE_WAVELENGTHS = {
    "H__1_486132A": 4861.32,
    "N__2_658345A": 6583.45,
    "O__3_500684A": 5006.84,
}
ATTENUATION_LAW = "CCM 89"
ATTENUATION_RV = 3.1
AV_PER_NH = 5.3e-22
...
def plot_profiles(m3d, x_pos, y_pos, title, velocity_limits=None):
    axis = plt.gca()
    lines = (
        ("H__1_486132A", r"H$\beta$", 5.0),
        ("N__2_658345A", r"[NII]$\lambda$6584", 5.0),
        ("O__3_500684A", r"[OIII]$\lambda$5007", 1.0),
    )
    for ref, label, scale in lines:
        profile = attenuated_profile(
            m3d, ref, LINE_WAVELENGTHS[ref], axis="x",
            law=ATTENUATION_LAW, r_v=ATTENUATION_RV, av_per_nh=AV_PER_NH,
        )[:, x_pos, y_pos]
        plt.plot(m3d.vel_tab, profile * scale, label=label)
...
def add_profiles_to_rgb(m3d, image_axis, ref, image_proj_axis=1, nx=20, ny=20):
    ...
    if isinstance(ref, (int, np.integer)):
        ref = m3d.m[0].emis_labels[ref]
    profiles = attenuated_profile(
        m3d, ref, LINE_WAVELENGTHS[ref], axis=profile_axis,
        law=ATTENUATION_LAW, r_v=ATTENUATION_RV, av_per_nh=AV_PER_NH,
    )
...
def other_plots(m3d, proj_axis, n_cut):
    hbmap = attenuated_projection(
        m3d, "H__1_486132A", LINE_WAVELENGTHS["H__1_486132A"], axis=proj_axis,
        law=ATTENUATION_LAW, r_v=ATTENUATION_RV, av_per_nh=AV_PER_NH,
    )
    niimap = attenuated_projection(
        m3d, "N__2_658345A", LINE_WAVELENGTHS["N__2_658345A"], axis=proj_axis,
        law=ATTENUATION_LAW, r_v=ATTENUATION_RV, av_per_nh=AV_PER_NH,
    )
    oiiimap = attenuated_projection(
        m3d, "O__3_500684A", LINE_WAVELENGTHS["O__3_500684A"], axis=proj_axis,
        law=ATTENUATION_LAW, r_v=ATTENUATION_RV, av_per_nh=AV_PER_NH,
    )
...
def run_profile(profile_file, include_xray=False):
    ...
    rgb_maps = [
        attenuated_projection(
            m3d, ref, LINE_WAVELENGTHS[ref], axis=1,
            law=ATTENUATION_LAW, r_v=ATTENUATION_RV, av_per_nh=AV_PER_NH,
        )
    ...
    rgb_axis.set_title(
        f"Internally attenuated RGB image with [NII] 6584 line profiles: {profile_stem}"
    )
    ...
    with warnings.catch_warnings():
        ...
        add_profiles_to_rgb(
            m3d, rgb_axis, ref=3, image_proj_axis=1, nx=20, ny=20
        )
    ...
    n2map, o3map, hbmap = [
        attenuated_projection(
            m3d, ref, LINE_WAVELENGTHS[ref], axis=PROJ_AXIS,
            law=ATTENUATION_LAW, r_v=ATTENUATION_RV, av_per_nh=AV_PER_NH,
        )
        for ref in ("N__2_658345A", "O__3_500684A", "H__1_486132A")
    ]
