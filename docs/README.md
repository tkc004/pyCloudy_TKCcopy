...
- [`Using_pyCloudy_3_spherical_profiles_rhd.py`](../python_examples/Using_pyCloudy_3_spherical_profiles_rhd/Using_pyCloudy_3_spherical_profiles_rhd.py): RHD-profile variant using `radial_profile_rhd.csv` from the RadHydropy [`DynamicStromgrenSpherePhotoheating20pc1D`](https://github.com/tkc004/RadHydropy/tree/main/example/DynamicStromgrenSpherePhotoheating20pc1D) simulation and an ionizing photon rate of `10^49 s^-1`; see [`Using_pyCloudy_3_spherical_profiles_rhd.md`](Using_pyCloudy_3_spherical_profiles_rhd.md).
- [`Using_pyCloudy_3_spherical_profiles_rhd_wind.py`](../python_examples/Using_pyCloudy_3_spherical_profiles_rhd_wind/Using_pyCloudy_3_spherical_profiles_rhd_wind.py): RHD stellar-wind profile variant using `radial_profile_rhd_wind.csv` from the RadHydropy [`DynamicStromgrenSpherePhotoheating20pcStellarWind1D`](https://github.com/tkc004/RadHydropy/tree/main/example/DynamicStromgrenSpherePhotoheating20pcStellarWind1D) simulation, generated with `10^-6 Msun/yr`, `1000 km/s`, and `q(H)=10^49 s^-1`. Cloudy preprocesses the ionization/emission structure, and C3D applies the simulated velocity for 3D diagnostics; see [`Using_pyCloudy_3_spherical_profiles_rhd_wind.md`](Using_pyCloudy_3_spherical_profiles_rhd_wind.md).
- [`Using_pyCloudy_3_spherical_profiles_rhd_wind_temperature.py`](../python_examples/Using_pyCloudy_3_spherical_profiles_rhd_wind_temperature/Using_pyCloudy_3_spherical_profiles_rhd_wind_temperature.py): Batch RHD stellar-wind workflow reading simulated `RADIUS_PC`, `VELOCITY_KMS`, `DENSITY_CM3`, and `TEMP_K` from CSV files in `radial_profiles/`; Cloudy preprocesses density/temperature with `q(H)=10^49 s^-1`, then C3D applies velocity for 3D structures and line profiles. Each profile has isolated output directories; see [`Using_pyCloudy_3_spherical_profiles_rhd_wind_temperature.md`](Using_pyCloudy_3_spherical_profiles_rhd_wind_temperature.md).
